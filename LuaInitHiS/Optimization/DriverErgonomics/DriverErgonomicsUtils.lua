...
function getTControlOfStaticGeometryByName(name)
	local geoRoot = Ips.getGeometryRoot();
	local obj = geoRoot:findFirstExactMatch(name):toPositionedTreeObject();
	local tControl = obj:getTControl();
	return tControl;
end
function getTControlOfActiveObjectByName(name)
	local objectsRoot = Ips.getActiveObjectsRoot();
	local obj = objectsRoot:findFirstExactMatch(name):toPositionedTreeObject();
	local tControl = obj:getTControl();
	return tControl;
end
...
function getGender(name)
	local first_part = name:match("^([^_]+)");
	return first_part;
end


function deleteRestOfTimelines()
	local root = Ips.getProcessRoot();
	local obj = root;
	while (not(obj == nil)) do
		local child = obj:getFirstChild();
		while (not(child == nil)) do
			if (child:getType() == "OperationSequence") then
				local timelineChild = child:getFirstChild();
				while (not(timelineChild == nil)) do
					Ips.deleteTreeObject(timelineChild);
					timelineChild = child:getFirstChild();
				end
			end
