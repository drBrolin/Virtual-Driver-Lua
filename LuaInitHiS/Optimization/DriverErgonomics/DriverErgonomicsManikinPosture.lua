...
function getDriverHiptoEyeAngleFunction(iMani) -- Right/LeftHip to Eyeside (mid-hip to center eye in Reed)
	...
	local hip2eyeAngle = math.deg(math.atan(xyProject/midHip2EyesideVec[2]))-offsetHip2Eye[iMani+1];
...
function getDriverHeadAngleFunction(iMani) -- AtlantoAxial to Eyeside (Tragion to center eye in Reed)
	local root = Ips.getActiveObjectsRoot();
	local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);
	...
	local headAngle = math.deg(math.atan(AA2EyesideVec[2]/xyProject))-offsetHead[iMani+1];
	--print("Head angle: " ..tostring(headAngle));

	return headAngle; 
end

function getDriverNeckAngleFunction(iMani) -- T1T2 to AtlantoAxial (C7T1 to Tragion in Reed)
	local root = Ips.getActiveObjectsRoot();
	local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);
	...
	local neckAngle = math.deg(math.atan(xyProject/C7T12AAVec[2]))-offsetNeck[iMani+1];
	--print("Neck angle: " ..tostring(neckAngle));

	return neckAngle;
end

function getDriverThoraxAngleFunction(iMani) -- (T12L1 to C7T1 in Reed)
	local root = Ips.getActiveObjectsRoot();
	local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);
	...
	local thoraxAngle = math.deg(math.atan(xyProject/T12L1C7T1Vec[2]))-offsetThorax[iMani+1];
	--print("Thorax angle: " ..tostring(thoraxAngle));

	return thoraxAngle; 
end

function getDriverAbdomenAngleFunction(iMani) --  (L5S1 to T12L1 in Reed)
	local root = Ips.getActiveObjectsRoot();
	local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);
	...
	local abdomenAngle = math.deg(math.atan(xyProject/L5S1toT12L1Vec[2]))-offsetAbdomen[iMani+1];
	--print("Abdomen angle: " ..tostring(abdomenAngle));

	return abdomenAngle; 
end

function getDriverPelvisAngleFunction(iMani)
	local root = Ips.getActiveObjectsRoot();
	local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]); 
	...
	local pelvisAngle = math.deg(math.atan(xyProject/midHip2L5S1Vec[2]))-offsetPelvis[iMani+1];
	--print("Pelvis angle: " ..tostring(pelvisAngle));
	
	return pelvisAngle; 	
end

function getDriverTorsoAngleFunction(iMani)
	-- local root = Ips.getActiveObjectsRoot();
	-- local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);
...
function getDriverThighAngleFunction(iMani)
	local root = Ips.getActiveObjectsRoot();
	local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);
	...
	local thighAngle = math.deg(math.atan(diffVec[2]/xyProject))-offsetThigh[iMani+1];
...
function getDriverRightKneeAngleFunction(iMani)
	local root = Ips.getActiveObjectsRoot();
	local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);

	local RightKneeAngle = fam:getJointAngleForManikin(iMani,"RightKnee");
	local RightKneeAngDeg = 180-math.deg(RightKneeAngle[0])-offsetKnee[iMani+1];
	--print("RightKneeAngle: " ..tostring(RightKneeAngDeg));
	
	return RightKneeAngDeg; 
end

function getDriverLeftKneeAngleFunction(iMani)
	local root = Ips.getActiveObjectsRoot();
	local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);

	local LeftKneeAngle = fam:getJointAngleForManikin(iMani,"LeftKnee");
	local LeftKneeAngDeg = 180-math.deg(LeftKneeAngle[0])-offsetKnee[iMani+1];
	--print("LeftKneeAngle: " ..tostring(LeftKneeAngDeg));	
	
	return LeftKneeAngDeg;
end

function getDriverRightElbowAngleFunction(iMani)
	-- local root = Ips.getActiveObjectsRoot();
	-- local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);
...
function getDriverLeftElbowAngleFunction(iMani)
	-- local root = Ips.getActiveObjectsRoot();
	-- local fam = ManikinSimulationController():getManikinFamily(ManikinSimulationController():getManikinFamilyIDs()[0]);
